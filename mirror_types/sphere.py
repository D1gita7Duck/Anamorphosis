...
def ray_sphere_intersection(V : NDArray, d : NDArray, R : float):
    """
    Computes ray and sphere intersection.
    Inputs:
        V : viewpoint coordinates
        d : unit vector from V to A (incident ray) A being the virtual image point
        R : radius of sphere centered at origin
    Outputs:
        None if there is no possible intersection
        NDArray containing the point of intersection
    """
    # include z it becomes ray and sphere intersection
    Vx, Vy, Vz = V
    dx, dy, dz = d

    a = dx**2 + dy**2 + dz**2
    b = 2 * (Vx*dx + Vy*dy + Vz*dz)
    c = Vx**2 + Vy**2 + Vz**2 - R**2
...
def mesh_within_sphere(vertices, r : float) -> bool:
    r2 = r*r
...
def move_mesh_inside_sphere(mesh : trimesh.Trimesh, R : float):
    """
    ...
    Returns:
        ...
        None if no problemo
    """
    bounding_radius = mesh.bounding_sphere.primitive.radius
    c_x, c_y, c_z = mesh.bounding_sphere.primitive.center
    print(f"Bounding sphere radius {bounding_radius}")
    print(f"Bounding sphere centre {c_x}, {c_y}")
...
class SphericalMirror:
    ...
    def generate(self):
        ...
        output_f_name = os.path.join(output_dir_name, "ana_sph_"+self.f_mesh.metadata['name']) # the metadata dict gets cleared after copying (idk why)
        
        if not mesh_within_sphere(self.f_mesh.vertices, self.R):
            print("Mesh not within cylinder. Trying to shift it...")
            move_mesh_inside_sphere(self.f_mesh, self.R)
        ...
        print("Mesh within sphere T/F: ", mesh_within_sphere(f_mesh_vertices, self.R))
        ...
        for A in f_mesh_vertices:
            ...
            I = ray_sphere_intersection(self.V, d, self.R)
            if I is None:
                continue
            
            # adding z component is sphere normal
            n = np.array([I[0]-C[0], I[1]-C[1], I[2]-C[2]])
